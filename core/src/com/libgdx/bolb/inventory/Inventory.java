	...
	public void sort() {
		...
		super.sort(new Comparator<ItemSlot>() {
			...
			public int compare(ItemSlot slot1, ItemSlot slot2) {
			    if(slot1.isEmpty()) return -1; // First slot is empty, so we return -1 because it goes AFTER the second one
			    if(slot2.isEmpty()) return 1; // Second slot is empty, so we return 1 because the first one goes BEFORE.
